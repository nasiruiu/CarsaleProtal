    ...
    RowsGroup.prototype = {
        ...
        _mergeColumn: function (iStartRow, iFinishRow, columnsIndexes) {
            ...
            for (iRow = iStartRow + 1; iRow <= iFinishRow; ++iRow) {
                //if (iRow == iFinishRow) {
                //    $(columnNodes[iRow]).parent().addClass("finish-row");
                //}
                if (columnValues[iRow] === columnValues[newSequenceRow]) {
                    $(columnNodes[iRow]).hide()
                } else {
                    if (((iRow - 1) - newSequenceRow + 1) > 1) {
                        $(columnNodes[iRow - 1]).parent().addClass("finish-row");
                        $(columnNodes[newSequenceRow]).parent().addClass("start-row");
                    }
